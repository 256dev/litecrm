...
<div class="container py-3">
        ...
        <div class="row col-12 col-lg-10 mx-auto">
            <div class="col-12 col-sm-6 mb-2">
                ...
                <input aria-label="Имя сотруника"
                       class="form-control {{ $errors->has('username')?'is-invalid':'' }}" 
                       value="{{ old('username')??($item->name??'') }}" required
                       id="username" type="text" name="username" placeholder="Имя сотруника"
                >
                @error('username')
            ...
            <div class="col-12 col-sm-6 mb-2">
                ...
                <input aria-label="Паспортные данные сотрудника"
                       ...
                       id="passport" type="text" name="passport" placeholder="Паспортные данные"
                >
                @error('passport')
                    <div><small class="text-danger">{{ $message }}</small></div>
                @enderror
            ...
            <div class="col-6 col-md-3 mb-2">
                ...
                <input aria-label="ИНН"
                       class="form-control {{ $errors->has('itin')?'is-invalid':'' }}" 
                       id="itin" type="text" placeholder="ИНН" name="itin"
                       value="{{ old('itin')??($item->itin??'') }}"
                >
                @error('itin')
                    <div><small class="text-danger">{{ $message }}</small></div>
                @enderror
            ...
            <div class="col-6 col-md-3 mb-2">
                ...
                <select id="role" aria-label="Должность" class="form-control" name="role">
                    ...
                    @foreach ($roles as $role)
                        <option value="{{ $role->id }}" {{ old('role')?($role->id == old('role')?'selected':''):((isset($edit) && $role->id == $item->role_id)?'selected':'') }}>{{ $role->name }}</option>
                    @endforeach
                </select>
                @error('role')
                    <div><small class="text-danger">{{ $message }}</small></div>
            ...
            <div class="col-6 col-md-3 mb-2">
                ...
                <input aria-label="Принят"
                       class="form-control {{ $errors->has('hired_date')?'is-invalid':'' }}"
            ...
            <div class="col-6 col-md-3 mb-2">
                ...
                <input aria-label="Уволен"
                       class="form-control {{ $errors->has('fired_date')?'is-invalid':'' }}" 
                       value="{{ old('fired_date')??($item->rired_date??'') }}"
                       id="fired_date" type="date" name="fired_date"
                >
                @error('fired_date')
                    <div><small class="text-danger">{{ $message }}</small></div>
                @enderror
            ...
            <div class="col-12 col-lg-6 text-md-center mb-2">
                ...
                <textarea aria-label="Квалификация" rows="2" name="qualification" placeholder="Квалификация" class="form-control {{ $errors->has('qualification')?'is-invalid':'' }}">{{ old('qualification')??($item->qualification??'') }}</textarea>
                @error('qualification')
                    <div><small class="text-danger">{{ $message }}</small></div>
                @enderror
            </div>
            <div class="col-12 col-lg-6 text-md-center mb-2">
                ...
                <textarea aria-label="Комментарий" rows="2" name="comment" placeholder="Комментарий"class="form-control {{ $errors->has('comment')?'is-invalid':'' }}">{{ old('comment')??($item->comment??'') }}</textarea>
                @error('comment')
                    <div><small class="text-danger">{{ $message }}</small></div>
                @enderror
            </div>
